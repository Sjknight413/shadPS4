...
void CheckUpdate::CheckForUpdates(const bool showMessage) {
    ...
    while (checkName) {
        ...
        if (updateChannel == "Nightly") {
            url = QUrl("https://api.github.com/repos/Sjknight413/shadPS4/releases");
            checkName = false;
        } else if (updateChannel == "Release") {
            url = QUrl("https://api.github.com/repos/Sjknight413/shadPS4/releases/latest");
            checkName = false;
        } else {
            if (Common::isRelease) {
...
void CheckUpdate::requestChangelog(const QString& currentRev, const QString& latestRev,
    ...
    QString compareUrlString =
        QString("https://api.github.com/repos/Sjknight413/shadPS4/compare/%1...%2")
            .arg(currentRev)
            .arg(latestRev);
